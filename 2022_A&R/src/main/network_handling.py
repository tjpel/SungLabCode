...
deg_list, gene_dict  = GSH.gene_file_to_gene_dict_and_list(filtered_deg_file, "\t", "gene")

topology_dict = NF.topology_file_to_dict(topology_file)
sub_topology_dict = NF.create_sub_topology(deg_list, topology_dict)

print(f"Info :: Original Topology > {len(topology_dict.keys())}")
print(f"Info :: Sub Topology > {len(sub_topology_dict.keys())}")
